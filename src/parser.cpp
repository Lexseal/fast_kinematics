...
size_t Parser::find_num_of_joints(JointTreePtr tip) {
...
JointTreePtr Parser::find_link_parent(JointTreePtr root, std::string eef_name, bool verbose) {
  JointTreePtr tip;
  std::queue<JointTreePtr> q;
  q.push(root);
  while (!q.empty()) {
    JointTreePtr node = q.front();
    q.pop();
    if (verbose) {
...
void Parser::prepare_repr(size_t num_of_robots, JointTreePtr tip, float **data, size_t **cum_idx) {
